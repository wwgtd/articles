...
import { createArticle, saveArticles, saveArticle, updateArticle, deleteArticle, writeError, loadingArticleOn, loadingArticleOff } from './articlesActions'
import { createCategory, saveCategories, updateCategory, deleteCategory, loadingCategoryOn, loadingCategoryOff } from './categoriesActions'
import { loadingCommentOn, loadingCommentOff } from './commentsActions'
import { registerUser, loginUser, logoutUser, userLoadingOn, userLoadingOff, writeLoginError } from './userActions'
...
let article_id = Object.keys(mock_db.articles).length !== 0 ? Object.keys(mock_db.users).length : 0;
let category_id = Object.keys(mock_db.category).length !== 0 ? Object.keys(mock_db.users).length : 0;
let comment_id = Object.keys(mock_db.comments).length !== 0 ? Object.keys(mock_db.comments).length : 0;
let user_id = Object.keys(mock_db.users).length !== 0 ? Object.keys(mock_db.users).length : 0;
...
const mock_api: API = {
  articles: {
    create: (params: ISyncParams) => new Promise<ISyncArticleAnswer>((res, rej) => {
      if (params.auth.token !== "1") res({success: false});
      mock_db.articles = {
        ...mock_db.articles,
        [article_id]: {id: article_id, ...params.data, created_at: new Date(Date.now()).toString(), updated_at: new Date(Date.now()).toString() }
      };
    ...
    getAll: () => new Promise<{ [index: number]: IArticle }>((res, rej) => {
    ...
    getOne: (id: number) => new Promise<IArticle>((res, rej) => {
      ...
      for (let prop in mock_db.comments) {
        if (mock_db.comments[prop].article_id === id) {
          ...
          target.comments.push({...mock_db.comments[prop], user: user_info});
        };
      };
      console.log(target);
      res(target);
    ...
    update: (params: ISyncParams) => new Promise<ISyncArticleAnswer>( (res, rej) => {
      if (params.auth.token !== "1") res({success: false});
      if (mock_db.articles[params.data.id!] === undefined) res({success: false});
      mock_db.articles[params.data.id!] = {...mock_db.articles[params.data.id!], ...params.data, updated_at: new Date(Date.now()).toString()};
      res({success: true, data: mock_db.articles[params.data.id!]});
    }),

    delete: (params: { auth: IAuthData, id: number}) => new Promise<ISuccessAnswer>( (res, rej) => {
      if (params.auth.token !== "1") res({success: false});
  ...
  category: {
    create: (params: ISyncParams) => new Promise<ICategorySyncAnswer>((res, rej) => {
      if (params.auth.token !== "1") res({success: false});
      mock_db.category[category_id] = { id: category_id, name: params.data.name };
      const target: ICategory = { ...mock_db.category[category_id]};
      category_id++;
      res({success: true, data: target});
    }),

    get: () => new Promise<{[index: number]: ICategory}>((res, rej) => {
      res(mock_db.category);
    }),

    update: (params: ISyncParams) => new Promise<ICategorySyncAnswer>((res, rej) => {
      if (params.auth.token !== "1") res({success: false});
  ...
  comments: {
    create: (params: ISyncParams) => new Promise<ICommentSyncAnswer>((res, rej) => {
      if (params.auth.token !== "1") res({success: false});
      const id = comment_id;
      mock_db.comments[id] = {id, body: params.data.body, user_id: params.data.user_id!, article_id: params.data.article_id,
        created_at: new Date(Date.now()).toString(), updated_at: new Date(Date.now()).toString()
       };
      comment_id++;
    ...
    update: (params: ISyncParams) => new Promise<ICommentSyncAnswer>((res, rej) => {
      if (!mock_db.comments[params.data.id!] || params.auth.token !== "1") {
        res({success: false});
     }
      mock_db.comments[params.data.id!] = { ...mock_db.comments[params.data.id!], ...params.data, updated_at: new Date(Date.now()).toString() };
      res({success: true});
    })
  },

  users: {
    register: (payload: { name: string, password: string, email: string }) => new Promise<ISuccessAnswer>((res, rej) => {
      if (mock_db.users[payload.email] !== undefined) { res({success: false}); }
      mock_db.users[payload.email] = { ...payload, id: user_id, access_level: "10", created_at: new Date(Date.now()).toString(),
      updated_at: new Date(Date.now()).toString(), activated: true};
      user_id++;
    ...
    login: (payload: {email: string, password: string}) => new Promise<ILoginAnswer>((res,rej) => {
      if (mock_db.users[payload.email]) {
        if (mock_db.users[payload.email].password === payload.password) {
         res({ success: true, auth_data: { id: mock_db.users[payload.email].id, token: "1" }, user_data: mock_db.users[payload.email] });
       } else {
    ...
    logout: ( payload: IAuthData ) => new Promise<ISuccessAnswer>((res, rej) => {
      res({success: true});
    })
  }
}
...
function getAPI(api: API) {
  const articles = {
    create(payload: {title: string, body: string, category_id: number}) {
      return (dispatch: any, getState: any) => {
        ...
        const request_data: ISyncParams = {auth, data: { ...payload, user_id: auth.id}};
    ...
    update(payload: {id: number, title?: string, body?: string, category_id?: number}) {
      return (dispatch: any, getState: any) => {
        dispatch(loadingArticleOn());
        const auth = getState().users.auth_data;
  ...
  const category = {
    create: (name: string) => (dispatch: any, getState: any) => {
      ...
      const auth = getState().users.auth_data;
      const request_data: ISyncParams = {auth, data: { name, user_id: auth.id }};
    ...
    update: (params: {id: number, newName: string}) => (dispatch: any, getState: any) => {
      dispatch(loadingCategoryOn());
      const auth = getState().users.auth_data;
      const request_data: ISyncParams = {auth, data: { id: params.id, name: params.newName }};
  ...
  const comments = {
    create: (params: { article_id: number, body:string}) => (dispatch: any, getState: any) => {
      dispatch(loadingCommentOn());
      const auth = getState().users.auth_data;
      const request_data: ISyncParams = {auth, data: { ...params, user_id: auth.id}};
    ...
    update: (params: {id: number, body: any}) => (dispatch: any, getState: any) => {
     dispatch(loadingCommentOn());
     const auth = getState().users.auth_data;
  ...
  const users = {
    login: (payload: {email: string, password: string}) => (dispatch: any) => {
    ...
    register: (payload: { name: string, password: string, email: string }) => (dispatch: any) => {
      dispatch(userLoadingOn());
