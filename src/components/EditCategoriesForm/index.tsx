...
class EditCategoriesForm extends React.PureComponent<
  IEditCategoriesFormProps,
  IEditCategoriesFormState
> {
  constructor(props: IEditCategoriesFormProps) {
    super(props);
    this.state = {
      name: "",
      display: DisplayStatus.CreateNewCategory,
      id: 0,
      ok: false
    };
  }
  ...
  componentDidMount() {
    this.setState({
      ...this.state,
      id: parseInt(Object.keys(this.props.categories.data)[0])
    });
  }
  ...
  handleSubmit = (e: any) => {
    e.preventDefault();
    this.state.display === DisplayStatus.CreateNewCategory
        ...
        <div className="edit_categories_frame">
          ...
          <form onSubmit={this.handleSubmit} className="edit_categories_form">
            <div>
              ...
              <input
                size={15}
                className="new_category_name_input"
                type="text"
                name="name"
...
const mapDispatchToProps = (dispatch: any) => {
  return {
    ...
    update: (payload: { id: number; newName: string }) =>
      dispatch(api.category.update(payload)),
