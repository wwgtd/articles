...
class Comment extends React.PureComponent<ICommentProps, ICommentState> {
  ...
  changeDisplay = () => {
    let status = this.state.display === CommentDisplayStatus.Default ? CommentDisplayStatus.CommentUpdate : CommentDisplayStatus.Default;
  ...
  submitCommentUpdate = (e: any) => {
    e.preventDefault();
    this.props.updateComment({body: this.state.commentBody, id: this.props.id});
    this.changeDisplay();
  ...
  render() {
    ...
    <>
      <div className='article_comment_author'>
        ...
        { this.props.isLoggedAdmin ? <button className="edit_comment_btn" onClick={() => this.changeDisplay()}>Edit</button> : null }
      </div>
      <div className='article_comment_body'>
        ...
        {this.state.display === CommentDisplayStatus.Default ?
          <div className='article_comment_body_message'> {this.props.body} </div> :
          <form className='update_article_form' onSubmit={this.submitCommentUpdate}>
            <div className='update_article_form_body'>
              <textarea name='commentBody' value={this.state.commentBody} onChange={this.handleInput} />
            </div>
            <input className="update_article_form_submit_btn" type="submit" value="Update comment" />
          </form>}
      </div>
    </>
    )
...
const mapDispatchToProps = (dispatch: any) => {
  return {
    updateComment: (params: {id: number, body: string}) => dispatch(api.comments.update(params))
  }
}
