...
interface IRegisterProps {
  register: (payload: { name: string, password: string, email: string }) => boolean,
...
class RegistryForm extends React.PureComponent<IRegisterProps, IRegisterState> {
  ...
  handleSubmit = (e: any) => {
    ...
    const reEmail = new RegExp("^[A-Za-z0-9._%+-]+@[A-Z0-9-]+.+.[A-Z]{2,4}$", "i");
      ...
      return(
        <form className="loginForm" onSubmit={this.handleSubmit}>
          ...
          <div>
           ...
           {this.props.users.isRegistrySuccess ?  <p className='success'> Successfully registered </p> : undefined}
        </form>)
    }
}
...
const mapDispatchToProps = (dispatch: any) => {
  return {
    register: (payload: { name: string, password: string, email: string }) : boolean => dispatch(api.users.register(payload)),
