...
class CreateArticleForm extends React.PureComponent<ICreateArticleFromProps, ICreateArticleFromState> {
  ...
  handleSubmit = (e: any) => {
    e.preventDefault();
    this.props.create({ title: this.state.title, body: this.state.body, category_id: this.state.category_id });
    this.props.onClick();
  }
      ...
      <div className="create_article_frame">
        ...
        <form className='create_article_form' onSubmit={this.handleSubmit}>
          ...
          <div className='create_article_form_title'>
            ...
            <input className='title_input' type='text' name='title' value={this.state.title} onChange={this.handleInput} />
          </div>
          </div>
          <div className='create_article_form_body'>
            <label htmlFor='body'> Body</label>
            <textarea rows={8} cols={45} name='body' value={this.state.body} onChange={this.handleInput} />
          </div>
          <input className="create_article_form_submit_btn" type="submit" value="Create article" />
...
const mapDispatchToProps = (dispatch: any) => {
  return {
    create: (data: {body: string, title: string, category_id: number}) => dispatch(api.articles.create(data))
  }
}
